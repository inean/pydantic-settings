                ...
                for i, v in enumerate(updating_mapping):
                    if i < len(updated_mapping):
                        if isinstance(v, (list, dict)):
                            v = _deep_update(updated_mapping[i], v)
                        updated_mapping[i] = v
                    else:
    ...
    def explode_env_vars(
        ...
        def _transform_list_based_field(annotation: Annotated, values: dict[str, Any]) -> dict[str, Any] | list[Any]:
            ...
            for i in (str(i) for i in range(len(values))):
                if i not in values:
                    break
                if lenient_issubclass(get_origin(get_args(annotation)[0]), list):
                    result.append(_transform_list_based_field(get_args(annotation)[0], values[str(i)]))
                else:
                    result.append(values[str(i)])
            return result
